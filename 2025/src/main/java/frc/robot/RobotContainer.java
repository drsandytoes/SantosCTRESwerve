...
public class RobotContainer {
    ...
    private void configureLogging() {
        var logOptions = new DogLogOptions()
            .withCaptureDs(RobotConstants.Logging.captureDS)
            .withCaptureNt(RobotConstants.Logging.captureNT)
            .withLogExtras(RobotConstants.Logging.captureExtras)
            .withCaptureConsole(RobotConstants.Logging.captureConsole);

        if (RobotConstants.Logging.capturePDH) {
            DogLog.setPdh(new PowerDistribution());
        }

        DogLog.setOptions(logOptions);
        DogLog.setEnabled(RobotConstants.Logging.enabled);
    }
    ...
    private void configureBindings() {
        ...
        joystick.touchpad().onTrue(drivetrain.runOnce(() -> {
            System.out.println("Resetting forward (and testing log capture)");
            var alliance = DriverStation.getAlliance().orElse(Alliance.Blue);
            resetPoseForward(alliance);
        }));
    ...
    public void resetPoseForward(Alliance alliance) {
        if (alliance == Alliance.Red) { 
            ...
            DogLog.timestamp("RESETTING FORWARD: RED ALLIANCE");
            drivetrain.resetRotation(MathUtils.rotation.kHalf);
        } else {
            // Start at blue origina
            DogLog.timestamp("RESETTING FORWARD: BLUE ALLIANCE");
